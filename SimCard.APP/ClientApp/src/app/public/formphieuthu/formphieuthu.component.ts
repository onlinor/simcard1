...
export class FormphieuthuComponent implements OnInit, OnDestroy {
  ...
  onSubmit() {
    ...
    if (this.isATM && this.dataPhieuThu.loaiNganHang !== 'default') {
      this.dataPhieuThu.maPhieu =
        'PT' + ngayLap + '/' + this.dataPhieuThu.loaiNganHang;
    } else {
    ...
    if (this.isNewCashBook) {
      ...
      this.subscription = this.cashbookService
        .updateCashbook(this.idSelectedCashbook, this.dataPhieuThu)
        .subscribe(
          () => {
            this.emitDataPhieuThu.emit({ ...this.dataPhieuThu });
          },
